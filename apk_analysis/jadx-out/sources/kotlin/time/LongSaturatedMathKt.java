...
public final class LongSaturatedMathKt {
    ...
    public static final long m1764saturatingAddNuflL3o(long j, DurationUnit unit, long j2) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        long m1681toLongimpl = Duration.m1681toLongimpl(j2, unit);
        if (((j - 1) | 1) == Long.MAX_VALUE) {
            return m1763checkInfiniteSumDefinedPjuGub4(j, j2, m1681toLongimpl);
        }
        if ((1 | (m1681toLongimpl - 1)) == Long.MAX_VALUE) {
            return m1765saturatingAddInHalvesNuflL3o(j, unit, j2);
        }
        long j3 = j + m1681toLongimpl;
        return ((j ^ j3) & (m1681toLongimpl ^ j3)) < 0 ? j < 0 ? Long.MIN_VALUE : Long.MAX_VALUE : j3;
    }

    /* renamed from: checkInfiniteSumDefined-PjuGub4, reason: not valid java name */
    private static final long m1763checkInfiniteSumDefinedPjuGub4(long j, long j2, long j3) {
        if (!Duration.m1667isInfiniteimpl(j2) || (j ^ j3) >= 0) {
            return j;
        }
    ...
    private static final long m1765saturatingAddInHalvesNuflL3o(long j, DurationUnit durationUnit, long j2) {
        long m1638divUwyO8pc = Duration.m1638divUwyO8pc(j2, 2);
        long m1681toLongimpl = Duration.m1681toLongimpl(m1638divUwyO8pc, durationUnit);
        return (1 | (m1681toLongimpl - 1)) == Long.MAX_VALUE ? m1681toLongimpl : m1764saturatingAddNuflL3o(m1764saturatingAddNuflL3o(j, durationUnit, m1638divUwyO8pc), durationUnit, Duration.m1670minusLRDsOJo(j2, m1638divUwyO8pc));
    }

    private static final long infinityOfSign(long j) {
        return j < 0 ? Duration.INSTANCE.m1737getNEG_INFINITEUwyO8pc$kotlin_stdlib() : Duration.INSTANCE.m1736getINFINITEUwyO8pc();
    }

    public static final long saturatingDiff(long j, long j2, DurationUnit unit) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        if ((1 | (j2 - 1)) == Long.MAX_VALUE) {
            return Duration.m1688unaryMinusUwyO8pc(infinityOfSign(j2));
        }
    ...
    private static final long saturatingFiniteDiff(long j, long j2, DurationUnit durationUnit) {
        ...
        if (((j3 ^ j) & (~(j3 ^ j2))) < 0) {
            if (durationUnit.compareTo(DurationUnit.MILLISECONDS) < 0) {
                ...
                return Duration.m1671plusLRDsOJo(DurationKt.toDuration(j4, DurationUnit.MILLISECONDS), DurationKt.toDuration(j5, durationUnit));
            }
            return Duration.m1688unaryMinusUwyO8pc(infinityOfSign(j3));
        }
