    ...
    private final int decodeImpl(byte[] source, byte[] destination, int destinationOffset, int startIndex, int endIndex) {
        ...
        while (true) {
            ...
            if (i3 == i && i5 + 3 < endIndex) {
                int i6 = i5 + 4;
                int i7 = (iArr[source[i5] & UByte.MAX_VALUE] << 18) | (iArr[source[i5 + 1] & UByte.MAX_VALUE] << 12) | (iArr[source[i5 + 2] & UByte.MAX_VALUE] << 6) | iArr[source[i5 + 3] & UByte.MAX_VALUE];
                if (i7 >= 0) {
            ...
            int i9 = source[i5] & UByte.MAX_VALUE;
            int i10 = iArr[i9];
            if (i10 >= 0) {
                i5++;
                i4 = (i4 << 6) | i10;
