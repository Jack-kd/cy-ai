...
public class CapacitorHttpUrlConnection implements ICapacitorHttpUrlConnection {
    ...
    private void writeObjectRequestBody(JSObject jSObject) throws IOException, JSONException {
        ...
        try {
            ...
            while (keys.hasNext()) {
                ...
                dataOutputStream.writeBytes(URLEncoder.encode(next, Key.STRING_CHARSET_NAME));
                dataOutputStream.writeBytes("=");
                dataOutputStream.writeBytes(URLEncoder.encode(obj.toString(), Key.STRING_CHARSET_NAME));
                if (keys.hasNext()) {
                    dataOutputStream.writeBytes("&");
                }
            }
    ...
    private String buildDefaultAcceptLanguageProperty() {
        ...
        return !TextUtils.isEmpty(country) ? String.format("%s-%s,%s;q=0.5", language, country, language) : String.format("%s;q=0.5", language);
    }
