...
public class BridgeWebChromeClient extends WebChromeClient {
    ...
    public boolean onShowFileChooser(WebView webView, final ValueCallback<Uri[]> valueCallback, final WebChromeClient.FileChooserParams fileChooserParams) {
        ...
        boolean z2 = isCaptureEnabled && asList.contains(SelectMimeType.SYSTEM_IMAGE);
        if (isCaptureEnabled && asList.contains(SelectMimeType.SYSTEM_VIDEO)) {
            z = true;
        }
        if (z2 || z) {
    ...
    private File createImageFile(Activity activity) throws IOException {
        return File.createTempFile("JPEG_" + new SimpleDateFormat("yyyyMMdd_HHmmss").format(new Date()) + "_", PictureMimeType.JPG, activity.getExternalFilesDir(Environment.DIRECTORY_PICTURES));
    }
}
